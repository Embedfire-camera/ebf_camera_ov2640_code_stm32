...
{				
  ...
  uint8_t *pbmpbuf = bmpbuf;
	...
	hbmp.bmp_InfoHeader.biBitCount = 24;	 	/* 24bit色深 */
  ...
  memset(pbmpbuf, 0, sizeof(BMP_BUF_SIZE));
    ...
    /* 按照bmp刷新顺序整理图像数据，从左到右，从下到上，
       数据整理结束前，LTDC显存不得做任何修改 */
		for(ty = y0 + height - 1; hbmp.bmp_InfoHeader.biHeight; ty--)
		{
			...
			{
        rgb = RGB565toRGB888(LTDC_Read_Point(layer, tx, ty));
				pbmpbuf[pixcnt + 0] = (rgb >> 16) & 0xFF;
        pbmpbuf[pixcnt + 1] = (rgb >>  8) & 0xFF;
        pbmpbuf[pixcnt + 2] = (rgb >>  0) & 0xFF;
				pixcnt += 3;
			}
